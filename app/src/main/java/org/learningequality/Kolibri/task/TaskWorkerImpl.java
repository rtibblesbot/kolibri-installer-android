...
public class TaskWorkerImpl implements WorkerImpl<TaskWorkerImpl.Message> {
  ...
  public TaskWorkerImpl(UUID id, @NonNull Context context) {
    ...
    // Initialize Python if not already started (for worker process)
    if (!Python.isStarted()) {
      Python.start(new AndroidPlatform(context));
      // Initialize Kolibri environment in worker process
      KolibriEnvironmentSetup.initializeEnv(context);
    }
  }
  ...
  public boolean execute(String id, String arg) {
    try {
      ...
      // Call the task worker with the UUID
      PyObject result = taskworkerModule.callAttr("execute_job", id.toString());
