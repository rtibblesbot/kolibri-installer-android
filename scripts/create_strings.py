...
def generate_loading_pages(output_dir, version_text):
    """
...
def main():
    ...
    Expects version text as first command line argument (e.g., "0.19.0b2-0.1.6-debug").
    """
    if len(sys.argv) < 2:
        print("Error: Version text is required as first argument")
        print("Usage: python create_strings.py <version-text>")
        sys.exit(1)

    version_text = sys.argv[1].replace("-official", "")

    with tempfile.TemporaryDirectory() as temp_dir:
        generate_loading_pages(temp_dir, version_text)
